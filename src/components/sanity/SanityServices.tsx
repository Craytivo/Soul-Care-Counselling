  ...
  return (
    <section className="mt-12 grid gap-6 md:grid-cols-2">
      {activeServices.map((service: Service) => (
        <article key={service.slug.current} className="rounded-2xl bg-white p-6 ring-1 ring-charcoal/10 hover:ring-clay/30 transition-all duration-200">
          {/* Service Image */}
          {service.image && (
            <div className="mb-4">
              <Image
          ...
          {service.features && service.features.length > 0 && (
            <ul className="mt-4 space-y-1">
              {service.features.slice(0, 3).map((feature: string) => (
                <li key={feature} className="text-sm text-charcoal/70 flex items-center">
