...
const portableTextComponents = {
  block: {
    h2: (props: { children?: React.ReactNode }) => {
      ...
      return (
        <h2 id={slugify(text)} className="mt-10 mb-4 text-2xl md:text-3xl font-bold text-bark font-heading">
          {props.children}
        </h2>
      )
    },
    h3: (props: { children?: React.ReactNode }) => {
      const text = String(props.children || '')
      return (
        <h3 id={slugify(text)} className="mt-8 mb-3 text-xl md:text-2xl font-semibold text-bark font-heading">
          {props.children}
        </h3>
      )
    },
