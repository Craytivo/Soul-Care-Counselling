...
module.exports.handler = async function (event, context) {
  try {
    ...
    if (isMultipart && Busboy) {
      ...
      try {
        const busboy =
          typeof Busboy === 'function'
            ? Busboy({ headers: event.headers })
            : new Busboy({ headers: event.headers })
    ...
    for (const k of Object.keys(fields)) {
      ...
      htmlLines.push(
        `<div class="field"><div class="label">${safeKey}</div><div class="value">${safeVal}</div></div>`
      )
    }
    htmlLines.push('</div>')
    if (fileName) {
      htmlLines.push(
        `<div style="margin-top:12px"><strong>Attachment:</strong> ${fileName} (${fileMime}; ${fileBuffer ? fileBuffer.length : 0} bytes)</div>`
      )
    }
    ...
    if (fileBuffer && fileBuffer.length) {
      if (process.env.S3_BUCKET) {
        try {
          fileLink = await uploadBufferToS3({
            buffer: fileBuffer,
          ...
          htmlLines.push(
            `<div style="margin-top:12px"><a href=\"${fileLink}\" target=\"_blank\" rel=\"noopener noreferrer\">Download attachment: ${sanitizeFilename(fileName || fields.filename || 'attachment')}</a></div>`
          )
        } catch (e) {
          console.error('S3 upload failed:', e && e.message ? e.message : e)
          plainTextLines.push('[Attachment upload failed]')
          htmlLines.push(
            `<div style="margin-top:12px;color:#a33"><strong>Attachment upload failed</strong></div>`
          )
        }
      } else {
        // No S3 configured — attempt a no-credentials upload to transfer.sh as a temporary fallback.
        try {
          fileLink = await uploadBufferToTransfer({
            buffer: fileBuffer,
            filename: fileName || fields.filename,
            mimeType: fileMime,
          })
          ...
          htmlLines.push(
            `<div style="margin-top:12px;color:#a33"><strong>Attachment omitted</strong></div>`
          )
        }
      }
    }
    ...
    if (msg.attachments && msg.attachments.length) {
      console.log(
        'Final attachments to send:',
        msg.attachments.map((a) => ({ filename: a.filename, type: a.type }))
      )
    }
    ...
    if (!process.env.SENDGRID_API_KEY) {
      ...
      return {
        statusCode: 200,
        body: 'Parsed form (send skipped because SENDGRID_API_KEY is not set).',
      }
    }
    ...
    try {
      ...
      if (msg.attachments && msg.attachments.length) {
        console.warn(
          'Emergency mode: stripping attachments before send to avoid known SendGrid bug.'
        )
      }
      msg.attachments = []
      ...
      console.log(
        ...
        Array.isArray(response)
          ? response.map((r) => ({ statusCode: r.statusCode, headers: r.headers }))
          : response
      )
      ...
      const indication =
        errMsg.includes('Expected each attachment to contain a `filename`') ||
        (sendErr &&
          sendErr.response &&
          sendErr.response.body &&
          JSON.stringify(sendErr.response.body).includes('filename'))
      if (indication) {
        try {
          ...
          msg.html =
            (msg.html || '') +
            `<div style="margin-top:12px;color:#a33"><strong>Note:</strong> Attachment omitted due to upload/formatting issue. Please ask applicant to re-send if needed.</div>`
          // remove attachments and retry
          msg.attachments = []
          const retryResp = await sgMail.send(msg)
          console.log(
            'SendGrid retry response (no attachments):',
            Array.isArray(retryResp)
              ? retryResp.map((r) => ({ statusCode: r.statusCode, headers: r.headers }))
              : retryResp
          )
          return { statusCode: 200, body: 'Email sent successfully (attachment omitted).' }
        } catch (retryErr) {
          console.error(
            'Retry without attachments failed:',
            retryErr && retryErr.message ? retryErr.message : retryErr
          )
          if (retryErr && retryErr.response && retryErr.response.body) {
            try {
              console.error('Retry SendGrid response body:', JSON.stringify(retryErr.response.body))
            } catch (e) {
              /* ignore */
            }
          }
          return {
            statusCode: 502,
            body:
              'Error sending email (retry failed): ' +
    ...
    return {
      statusCode: 500,
      body: 'Error sending email: ' + (err && err.message ? err.message : String(err)),
    }
  }
}
